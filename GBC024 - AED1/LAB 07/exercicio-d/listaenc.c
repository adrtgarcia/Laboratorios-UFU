...
int contem_item (lista *l, int chave)
{
    if (l == NULL)
        return 2;                           //lista não alocada
    ...
    {
        if (n_lista->valor.mat == chave)
            return 0;                       //item está na lista

        n_lista = n_lista->prox;
    }

    return 1;                               //item não está na lista
}
...
lista *concatena (lista *l1, lista *l2)
{
    ...
    if ((l != NULL) && (l1 != NULL) && (l2 != NULL))
    {
        if (l1->inicio != NULL)
        {
            no *n_lista = l1->inicio;
            l->inicio = l1->inicio;

            while ((n_lista->prox != NULL))
                n_lista = n_lista->prox;

            n_lista->prox = l2->inicio;
        } else
            l->inicio = l2->inicio;

        l2->inicio = NULL;

        return l;                            //listas concatenadas
    }

    return NULL;                             //lista(s) não alocada(s)
}
...
void maior_nota (lista *l)
{
    if ((l != NULL) && (lista_vazia(l) == 0))
    {
        ...
        no *n_lista = l->inicio;
        maior = n_lista->valor.n1;

        while (n_lista != NULL)
        {
            n_lista = n_lista->prox;

            if (n_lista->valor.n1 > maior)
            {
                maior = n_lista->valor.n1;
                a = n_lista->valor;
            }
